...
## Architecture
frontend/ (Next.js 14)           backend/ (Express.js)├── app/                         ├── src/│   ├── page.tsx (Home)          │   ├── routes/       (HTTP layer)│   ├── login/                   │   ├── controllers/  (Request handling)│   ├── register/                │   ├── services/     (Business logic)│   ├── events/                  │   ├── repositories/ (Data access)│   │   ├── page.tsx (List)      │   ├── validators/   (Input validation)│   │   └── [id]/page.tsx        │   ├── middleware/    (Auth, errors)│   ├── bookings/                │   └── utils/        (Error classes)│   │   ├── page.tsx (List)      ├── prisma/│   │   └── [id]/page.tsx        │   ├── schema.prisma│   └── admin/                   │   └── seed.js│       ├── events/page.tsx      └── app.js│       └── bookings/page.tsx├── components/├── lib/│   ├── api/ (client, endpoints)│   └── hooks/ (useAuth, useEvents, useBookings)└── types/
## Data Models
...
| **eventDate** | DateTime | Must be a future date |
...
Load these resources dynamically based on the current active task:
...
Would you like me to create the template files for any of those sub-files (like business-rules.md or ui-selectors.md)? Let me know how you want to proceed!