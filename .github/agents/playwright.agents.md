...
`playwright-best-practices` skill — Your coding standards. Follow every rule.
`eventhub-domain` skill — Overview and data models
`eventhub-domain` sub-files — Read `./ui-selectors.md` for selectors, `./business-rules.md` for assertions, `./user-flows.md` for test steps
`tests/*.spec.js` — Existing tests to match patterns
`frontend/app/`, `frontend/components/` — Verify selectors exist in actual source code
Task
